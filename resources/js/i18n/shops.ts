export const shops = {
    fr: {
        identity: 'Identité',
        settings: 'Réglages',
        stats: {
            products: 'Produits',
            stockUnits: 'Unités en stock',
            stockValue: 'Valeur du stock',
            lowStock: 'Stock bas',
        },
        transfer: {
            title: 'Transférer des produits',
            help: "Un produit appartient à une seule boutique : la quantité sort d'ici et entre chez la destination, sur le produit de même SKU ou de même nom — créé là-bas s'il n'existe pas encore.",
            destination: 'Boutique de destination',
            selectDestination: '-- Choisir une boutique --',
            quantity: 'Quantité',
            submit: 'Transférer',
            nothingToTransfer: 'Aucun produit en stock à transférer.',
            searchPlaceholder: 'Rechercher un produit par nom ou SKU...',
            emptyCart: 'Recherchez un produit ci-dessus pour l\'ajouter au transfert.',
            history: 'Historique des transferts',
            reference: 'Référence',
            direction: 'Sens',
            sentTo: 'Envoyé vers',
            receivedFrom: 'Reçu de',
            by: 'Par',
            cancel: 'Annuler le transfert',
            cancelled: 'Annulé',
            cancelConfirm: 'Annuler le transfert :reference ? La marchandise repartira vers la boutique d\'origine.',
            noTransfers: 'Aucun transfert.',
        },
        title: 'Boutiques',
        columns: {
            name: 'Nom',
            slug: 'Slug',
            phone: 'Téléphone',
            email: 'Email',
            city: 'Ville',
            status: 'Statut',
            actions: 'Actions',
        },
        titleLong: 'Mes Boutiques',
        subtitle: 'Gérez vos boutiques et points de vente',
        limitReached: 'Limite atteinte. Passez à un plan supérieur pour ajouter des boutiques.',
        remaining: (n: number) => `${n} boutique${n > 1 ? 's' : ''} restante${n > 1 ? 's' : ''}`,
        empty: {
            title: 'Aucune boutique',
            description: 'Commencez par créer votre première boutique pour gérer vos produits et ventes.',
            cta: 'Créer ma première boutique',
        },
        form: {
            location: 'Localisation',
            contactSection: 'Contact',
            additionalInfo: 'Informations complémentaires',
            fullAddress: 'Adresse complète',
            taxIdLabel: 'ICE / N° Fiscal',
            createShop: 'Créer la boutique',
            editTitle: 'Modifier la boutique',
        },
        actions: {
            new: 'Nouvelle boutique',
            edit: 'Modifier',
            delete: 'Supprimer',
            view: 'Voir',
        },
        limitTooltip: 'Limite de boutiques atteinte. Passez à un plan supérieur.',
        emptyMessage: 'Aucune boutique trouvée',
        deleteConfirm: (name: string) => `Êtes-vous sûr de vouloir supprimer la boutique "${name}" ?`,
    },
    en: {
        identity: 'Identity',
        settings: 'Settings',
        stats: {
            products: 'Products',
            stockUnits: 'Units in stock',
            stockValue: 'Stock value',
            lowStock: 'Low stock',
        },
        transfer: {
            title: 'Transfer products',
            help: 'A product belongs to one shop only: the quantity leaves here and arrives at the destination, on the product with the same SKU or name — created there if it does not exist yet.',
            destination: 'Destination shop',
            selectDestination: '-- Choose a shop --',
            quantity: 'Quantity',
            submit: 'Transfer',
            nothingToTransfer: 'No product in stock to transfer.',
            searchPlaceholder: 'Search a product by name or SKU...',
            emptyCart: 'Search for a product above to add it to the transfer.',
            history: 'Transfer history',
            reference: 'Reference',
            direction: 'Direction',
            sentTo: 'Sent to',
            receivedFrom: 'Received from',
            by: 'By',
            cancel: 'Cancel transfer',
            cancelled: 'Cancelled',
            cancelConfirm: 'Cancel transfer :reference? The goods will return to the originating shop.',
            noTransfers: 'No transfers.',
        },
        title: 'Shops',
        columns: {
            name: 'Name',
            slug: 'Slug',
            phone: 'Phone',
            email: 'Email',
            city: 'City',
            status: 'Status',
            actions: 'Actions',
        },
        titleLong: 'My Shops',
        subtitle: 'Manage your shops and points of sale',
        limitReached: 'Limit reached. Upgrade to a higher plan to add more shops.',
        remaining: (n: number) => `${n} shop${n > 1 ? 's' : ''} remaining`,
        empty: {
            title: 'No shops',
            description: 'Start by creating your first shop to manage your products and sales.',
            cta: 'Create my first shop',
        },
        form: {
            location: 'Location',
            contactSection: 'Contact',
            additionalInfo: 'Additional information',
            fullAddress: 'Full address',
            taxIdLabel: 'ICE / Tax ID',
            createShop: 'Create shop',
            editTitle: 'Edit shop',
        },
        actions: {
            new: 'New shop',
            edit: 'Edit',
            delete: 'Delete',
            view: 'View',
        },
        limitTooltip: 'Shop limit reached. Upgrade to a higher plan.',
        emptyMessage: 'No shops found',
        deleteConfirm: (name: string) => `Are you sure you want to delete shop "${name}"?`,
    },
} as const;
